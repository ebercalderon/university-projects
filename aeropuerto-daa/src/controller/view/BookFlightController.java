...
public class BookFlightController {
        ...
        flightsT.setRowFactory(tv -> {
            ...
            row.setOnMouseClicked(event -> {
                if (event.getClickCount() == 2 && (!row.isEmpty())) {
                    ...
                    try {
                        newTrip();
                        Place.setText("a: " + temp.getDestination().toString());
    ...
    void saveTicket(ActionEvent event) {
        if (validationTicket()) {
            ...
            txtFlightId.setText("Código de vuelo: " + selectedFlight.getId());
            txtTo.setText("Destino: " + selectedFlight.getDestination().name());
            txtSelctedSeat.setText(
                    "Asiento seleccionado : " + String.valueOf(selectSeat.getSeatLetter()) + selectSeat.getSeatNumber() + "");
            updateLuggageToTrip(trip);
            calculateTripPrice();
            txtTotalPrice.setText("Total:         $" + tripPrice);
            idTrip.setText(
                    "Id. de viaje " + "T" + airport.getLogged().getTrips().size() + "-" + ticket.getFlight().getId());

            modalTicket.close();
            dController.alert(Route.SUCCESS, "Billete guardado");
        } else {
            dController.alert(Route.ERROR, "Por favor, seleccione un asiento haciendo clic en él");
        }
    }
    ...
    private void onCLickActionSeat(MouseEvent e, Flight flight) {
        ...
        if (source instanceof GridPane) {
            ...
            if (tempSeat.getSeatState()) {
                dController.alert(Route.ERROR, "El asiento ya está ocupado, intente otro asiento");
            } else {
    ...
    public void saveLuggage(ActionEvent event) {
        if (luggageValidation()) {
            if (cbType.getValue() == SpecialLuggageType.ESTANDAR) {
            ...
            txtTotalPrice.setText("Total:         $" + tripPrice);
            modal.close();
            dController.alert(Route.SUCCESS, "Equipaje añadido al viaje");
        } else {
            dController.alert(Route.ERROR, "Rellene todos los campos");
        }
    }
    ...
    private void calculateTripPrice() {
        int calculate = 0;
        if (ticket != null) {
            txtDate.setText("La fecha: " + ticket.getFlight().getArrivalDate());
            txtPriceflight.setText("Vuelo:        $" + ticket.getFlightPrice());
        ...
        txtPriceLuggage.setText("Equipaje: $" + lugPrice);
        tripPrice = calculate;
    }
    ...
    void buy(ActionEvent event) throws IOException {
        if (tripValidation()) {
            ...
            if (isUpdateMode) {
                ...
                idTrip.setText("Id. de viaje" + "valor " + id);
                ...
                dController.alert(Route.SUCCESS, "Viaje reservado");
                selectedFlight = null;
                selectSeat = null;
                trip = null;
            }
            ...
            dController.alert(Route.ERROR, "Seleccione un asiento y vuelva a intentarlo");
        }
    }
