...
public class NodeUpcomingController {
    ...
    public void getData(Flight flight) {
        ...
        txtDuration.setText(hours + " horas " + minutes + " minutos");
    }

    public void initState(FlightState state) {
        if (state == FlightState.CANCELADO || state == FlightState.RETRASADO) {
            error.toFront();
            error.setText(state.toString());
        } else if (state == FlightState.HECHO || state == FlightState.VOLANDO) {
            active.toFront();
